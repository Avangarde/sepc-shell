    ...
    while ((w = words[i++]) != 0) {
        switch (w[0]) {
            ...
            default:
                ...
                if (glob(words[i - 1], GLOB_TILDE | GLOB_BRACE | GLOB_NOCHECK, NULL, &pglob) == 0) {
                    cmd = xrealloc(cmd, (cmd_len + pglob.gl_pathc + 2) * sizeof (char *));
                    
                    int j;
                    for (j = 0; j < pglob.gl_pathc; j++) {
                        cmd[cmd_len] = xmalloc((strlen(pglob.gl_pathv[j]) + 1) * sizeof (char));
                        strcpy(cmd[cmd_len++], pglob.gl_pathv[j]);
                        cmd[cmd_len] = 0;
                    } 
                    //cmd[cmd_len++] = w;
                    //cmd[cmd_len] = 0;
